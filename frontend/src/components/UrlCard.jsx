...
export default function UrlCard({ url, onDelete, onUpdate }) {
  ...
  const handleSuspendSave = (updated) => {
    onUpdate?.(updated);
  ...
  return (
    <>
              ...
              <AnimatePresence>
                {showMenu && (
                  ...
                  >
                    ...
                    <button
                      onClick={() => { setShowSuspend(true); setShowMenu(false); }}
                      className="w-full px-3 py-2 text-left text-sm text-surface-700 dark:text-dark-300 hover:bg-surface-200 dark:hover:bg-dark-700 flex items-center gap-2"
                    >
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <SuspendModal isOpen={showSuspend} onClose={() => setShowSuspend(false)} url={url} onSave={handleSuspendSave} />
    </>
  );
}